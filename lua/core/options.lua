...
local current_config_path = config_path .. "/lua/core/machine_specific.lua"
if not vim.uv.fs_stat(current_config_path) then
  local current_config_file = io.open(current_config_path, "wb")
  local default_config_path = config_path .. "/lua/templates/_machine_specific_default.lua"
...
local ok, _ = pcall(require, "core.machine_specific")
if not ok then
  -- machine_specific is optional, ignore if not found
end
